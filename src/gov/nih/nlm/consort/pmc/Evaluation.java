...
public class Evaluation {
	private static Logger log = Logger.getLogger(Evaluation.class.getName());	
	...
	private static Map<String,String> getLabels(String file) throws IOException  {
		List<String> lines = FileUtils.linesFromFile(file, "UTF-8");
		Map<String,String> labels = new HashMap<>();
		for (String l: lines) {
			String[] els = l.split("\\|");
			labels.put(els[0] + "_" + els[1], els[2]);
		}
		return labels;
	}
...
//			loss += CalcMASI.calcJ(s1, s2);
	...
	public static void main(String[] args) throws ParserConfigurationException, SAXException, IOException {
		...
		Map<String,String> predictions = getLabels(predict);
		Map<String,String> golds = getLabels(gold);
		
		ArrayList<String> a= new ArrayList<>();
		ArrayList<String> b = new ArrayList<>();
		for (String s: predictions.keySet()) {
			a.add(predictions.get(s));
			if (golds.get(s) == null ) 
				log.warning("Null instance");
			b.add(golds.get(s));
		}
		double masi = CalcMASI.calcMASI(a, b);
		double hammingLoss = hammingLoss(a,b);
		System.out.println("MASI : " + masi);
		System.out.println("Hamming loss : " + hammingLoss);
		
		for (String id: Constants.METHOD_ITEM_IDS) {
			ArrayList<String> binaryPredict = keepItemOnly(id,a);
			ArrayList<String>binaryGold = keepItemOnly(id,b);
			
			int TP = 0;
			int FP = 0;
			int FN = 0;
			for (int i=0; i < binaryPredict.size(); i++) {
				String p = binaryPredict.get(i);
				String g = binaryGold.get(i);
				if (p.equals("1") && p.equals(g))  TP++;
				if (p.equals("0") && g.equals("1")) FN++;
				else if (p.equals("1") && g.equals("0")) FP++; 
			}
